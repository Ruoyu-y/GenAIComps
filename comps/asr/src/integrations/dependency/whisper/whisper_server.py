...
async def audio_streaming(
    websocket: WebSocket,
    chunk_duration_ms: int = DEFAULT_CHUNK_DURATION_MS,  # duration of each audio chunk in milliseconds
    language: str = "auto",
    beam_size: int = 5,
    ...
    chunk_size = int((chunk_duration_ms * DEFAULT_FRAMES_PER_CHUNK ) / 1000) * BYTES_PER_SAMPLE
    logger.info(f"Audio chunk size: {chunk_size} bytes ({chunk_duration_ms}ms)")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # if we have data in buffer and timeout occurred, process it
                    if len(audio_buffer) > 0:
                        logger.info(f"Data receive timeout ({data_timeout_ms}ms), processing final {len(audio_buffer)} bytes of audio data")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--faster_whisper_model", type=str, default="small")
